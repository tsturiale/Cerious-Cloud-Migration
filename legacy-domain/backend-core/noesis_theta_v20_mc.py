"""
Noesis Theta Gate V20 MC — Merton Jump-Diffusion Monte-Carlo & Volatility Regime System
=======================================================================================
Production-grade system module using the Monte-Carlo simulation engine.

Features:
- Monte-Carlo simulation of Merton Jump-Diffusion SDE (price_binary_up).
...
- Stateful position tracker (NoesisThetaV20MCTracker) for real-time monitoring.
...
def compute_v20_mc_probability(
    spot: float,
    strike: float,
    sigma_annual: float,
    tau_years: float,
    ...
    """
    Computes P(S_T > K) using Monte-Carlo simulation of Merton Jump-Diffusion SDE.
    """
    if tau_years <= 1e-8:
...
class NoesisThetaV20MCTracker:
    """
    Stateful execution tracker for Noesis Theta Gate V20 MC.
    Maintains trade lifecycles, computes dynamic Merton MC probabilities, and checks stop conditions.
    """
    ...
    def evaluate_tick(
        ...
        prob_yes = compute_v20_mc_probability(
...
def run_v20_mc_signal_scan(
    ...
    prob_yes = compute_v20_mc_probability(spot, strike, sigma_blend, tau, jl, js)

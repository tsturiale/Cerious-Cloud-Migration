...
async def _run_system_warmup() -> None:
    ...
    try:
        intelligence = await run_intelligence(lambda: acme_intelligence(force_refresh=True))
...
async def ensure_warmup_task() -> asyncio.Task[None]:
    global WARMUP_TASK
...
def _compose_synthetic_history(asset: str, interval: str, limit: int) -> list[dict[str, Any]]:
    ...
    for left in left_rows:
        ...
        if not right:
            continue
        values = {
            key: _number(left[key]) + _number(right[key]) * multiplier
...
async def _refresh_historical_backfill(asset: str, interval: str, limit: int) -> None:
    ...
    try:
        async with HISTORICAL_REFRESH_SEMAPHORE:
            await asyncio.to_thread(_historical_backfill, asset, interval, limit)
    finally:
...
async def bars(asset: str, interval: str = "1m", limit: int = 1000) -> list[dict[str, Any]]:
    ...
    live = _live_bars(key, interval, requested_limit, include_forming=True)
    historical = _cached_historical_backfill(key, interval, requested_limit)
    if not historical:
        _ensure_historical_refresh(key, interval, requested_limit)
    return _merge_backfill(live, historical, interval, requested_limit) if historical else live
...
async def acme_lr27(symbol: str, fresh: bool = False) -> dict[str, Any]:
    ...
    interval = "30m"
    limit = 80
    live = _live_bars(key, interval, limit, include_forming=True)
    historical = _cached_historical_backfill(key, interval, limit)
    if fresh or not historical:
        historical = await asyncio.to_thread(_historical_backfill, key, interval, limit)
    merged = _merge_backfill(live, historical, interval, limit) if historical else live
    completed = _completed_bars(merged, interval)[-limit:]
    forming_bar = next((row for row in reversed(merged) if _bar_ts(row) and _bar_ts(row) + _interval_ms(interval) > int(time.time() * 1000)), None)
    closes = [_number(row.get("close")) for row in completed if row.get("close") is not None]
    if len(closes) < 27:
        raise HTTPException(status_code=503, detail=f"LR27 for {key} has {len(closes)} completed 30m bars; needs 27")

    band = _linear_regression_band(closes, 27, 2.0)
    updated_at = _bar_ts(completed[-1])
    quote = market_bus.quotes.get(key)
    last = quote.last if quote else closes[-1]
    forming_update_ms = int(_number((forming_bar or {}).get("lastUpdateMs"), _bar_ts(forming_bar or {}))) if forming_bar else 0
    return {
        "symbol": key,
        ...
        "lastTraded": last,
        "live": quote is not None,
        "source": "databento-rest-baseline-plus-live-rolling-30m",
    }
